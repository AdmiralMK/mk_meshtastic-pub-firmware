...
void eventAdmCmdsys_handle(const meshtastic_MeshPacket &mp, const char *args, char *reply, size_t replySize)
{
    ...
    bool &flag = (strcasecmp(scope, "pub") == 0) ? BotState::instance().flags.system_pub_enabled
                                                  : BotState::instance().flags.system_dm_enabled;
    ...
    if (strcasecmp(target, "all") == 0 || strcasecmp(target, ourIdStr) == 0) {
        ...
        snprintf(reply, replySize, "✅ System commands (%s) turned %s on node %s", scope, turnOn ? "ON" : "OFF", target);
    } else {
